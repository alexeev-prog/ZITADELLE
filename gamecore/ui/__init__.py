...
def print_player_panel(player, skip_submenu: bool = False):
    ...
    main_panel = (
        ...
        f"[blue]{player.lvl} LVL | {player.xp}/{player.xp_to_next} XP[/blue]\n"
        f"HP: [green]{player.hp:.1f}/{player.max_hp:.1f}[/green] {hp_bar}\n"
        f"Мана: [cyan]{player.mana:.1f}/{player.max_mana:.1f}[/cyan] {mana_bar}\n"
        f"СИЛА: [red]{player.power}[/red]  ЛОВКОСТЬ: [blue]{player.agility}[/blue]  МУДРОСТЬ: [magenta]{player.wisdom}[/magenta]\n"
        f"УРОН: [red]{player.damage:.1f}[/red]  ДЕНЬГИ: [yellow]{player.money}[/yellow]\n"
    ...
    spells = "\n".join(
        [f"[bold magenta]{name}[/bold magenta] - {spell.spell_desc} ({spell.mana_cost} маны)"
         for name, spell in player.spells.items()]
    )

    materials = "\n".join(
        [f"[bold green]{mat}[/bold green]: {count}" for mat, count in player.crafting_materials.items()]
    ) or "Нет"
