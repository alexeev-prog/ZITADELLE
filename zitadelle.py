    ...
    for passive_ability in player.passive_abilities:
        if passive_ability.param == "discount":
            ...
            for item in SHOP_ITEMS.keys():
                SHOP_ITEMS[item]["price"] = SHOP_ITEMS[item]["price"] - (
                    SHOP_ITEMS[item]["price"] / 100 * value
                )
...
def battle(player, multiplier=1):
    ...
    while enemy.hp > 0:
        if player.hp <= 0 and not has_fortitude:
            ...
            print(
                f"[[bold]СТОЙКОСТЬ[/bold]] Вы могли бы погибнуть, но смогли собраться с силами и получить {fortitude} HP!"
            )
            player.hp = fortitude
            has_fortitude = False
    ...
    for passive_ability in player.passive_abilities:
        if passive_ability.param == "mana_loot":
            print(
                f"[[bold]МАНИЯ МАНЫ[/bold]] Вы получаете {loot * passive_ability.value} маны!"
            )
    ...
    while True:
        ...
        if action == "0":
            ...
            if chance == 1:
                ...
                abilities_name = [
                    "Дизельный пропеллер",
                    "С вами быть больше не хочу",
                    "Крыша",
                    "Ловля",
                    "Пассивка для балбеса",
                    "Варенье",
                    "Пропеллер на дизеле",
                    "Мана из дизеля",
                    "Квак-квак",
                    ...
                    "Вы меня не унизили",
                    "Скукота",
                    "Покривлялся и улетел",
                    "ПУУУСК!",
                ]
                ability_param = [
                    "random_additional_param",
                    "discount",
                    "health_fortitude",
                    "mana_loot",
                ]
                ability_values = {
                    "random_additional_param": randint(
                        10 * player.lvl, 20 * player.lvl
                    ),
                ...
                ability = PassiveAbility(
                    name=choice(abilities_name), desc=desc, param=param, value=value
                )

                price = randint(100 * player.lvl, 300 * player.lvl) * player.lvl

                print(
                    f"Вам предложили за небольшую плату ([bold yellow]{price}[/bold yellow]) получить новую пассивную способность:"
                )
                ...
                if buy:
                    if player.money < price:
                        print("У вас [red bold]не хватает средств.[/red bold]")
                    else:
                        print("[green bold]Успешная покупка![/green bold]")
